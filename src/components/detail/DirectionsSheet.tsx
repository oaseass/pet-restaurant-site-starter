...
export function DirectionsSheet({ name, lat, lng, address }: DirectionsSheetProps) {
  ...
  return (
    <>
      ...
      <div className="hidden flex-wrap gap-2 md:flex">
        ...
        {links.tmapUrl ? (
          <a href={links.tmapUrl} className="inline-flex min-h-11 items-center gap-2 rounded-full border border-[var(--line)] bg-white px-5 py-2.5 text-sm font-black text-[var(--ink)]">
            <Navigation size={15} />
            티맵
          </a>
        ) : null}
        <button type="button" onClick={handleCopyAddress} disabled={!links.hasAddress} className="inline-flex min-h-11 items-center gap-2 rounded-full border border-[var(--line)] bg-white px-5 py-2.5 text-sm font-black text-[var(--ink)] disabled:cursor-not-allowed disabled:opacity-50">
          <Copy size={15} />
