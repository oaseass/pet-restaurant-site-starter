    ...
    <main className="mx-auto max-w-5xl px-5 py-8 sm:py-10">
      <section className="section-shell px-6 py-6 sm:px-8 sm:py-8">
        <div className="relative z-10 max-w-3xl">
          ...
          <p className="mt-4 text-sm leading-8 text-[var(--muted)] sm:text-base">예방접종 일정, 월 양육비, 사료 급여량처럼 자주 계산하는 항목을 빠르게 확인합니다.</p>
        </div>
      </section>

      <section className="mt-6 grid gap-4 md:grid-cols-3">
        {CALCULATOR_CARDS.map((card) => (
          <Link key={card.href} href={card.href} className="card rounded-[1rem] p-5 transition hover:border-[rgba(31,107,91,0.2)] hover:bg-[#fcfbf9]">
