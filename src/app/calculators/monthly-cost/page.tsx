    ...
    <main className="mx-auto max-w-4xl px-5 py-8 sm:py-10">
      <section className="section-shell px-6 py-6 sm:px-8 sm:py-8">
        <div className="relative z-10 max-w-3xl">
          ...
          <p className="mt-4 text-sm leading-8 text-[#655a53] sm:text-base">사료비, 병원비, 미용비, 유치원 비용까지 월별 예산을 대략적으로 잡을 수 있습니다.</p>
        </div>
      </section>
      ...
      <form className="mt-6 grid gap-4 rounded-[2rem] card p-6 md:grid-cols-2">
        ...
        <label className="space-y-2 text-sm font-bold text-[#4b423c]">나이(년)<input type="number" step="1" name="ageYears" defaultValue={input.ageYears} className="input" /></label>
        <label className="space-y-2 text-sm font-bold text-[#4b423c]">사료비<input type="number" name="foodCost" defaultValue={input.foodCost} className="input" /></label>
        <label className="space-y-2 text-sm font-bold text-[#4b423c]">간식비<input type="number" name="snackCost" defaultValue={input.snackCost} className="input" /></label>
        <label className="space-y-2 text-sm font-bold text-[#4b423c]">미용비<input type="number" name="groomingCost" defaultValue={input.groomingCost} className="input" /></label>
        <label className="space-y-2 text-sm font-bold text-[#4b423c]">병원비<input type="number" name="hospitalCost" defaultValue={input.hospitalCost} className="input" /></label>
        <label className="space-y-2 text-sm font-bold text-[#4b423c]">유치원/호텔비<input type="number" name="daycareCost" defaultValue={input.daycareCost} className="input" /></label>
        <label className="space-y-2 text-sm font-bold text-[#4b423c]">보험료<input type="number" name="insuranceCost" defaultValue={input.insuranceCost} className="input" /></label>
        <label className="space-y-2 text-sm font-bold text-[#4b423c]">기타<input type="number" name="otherCost" defaultValue={input.otherCost} className="input" /></label>
      ...
      <section className="mt-6 card rounded-[2rem] p-6">
        <div className="grid gap-4 md:grid-cols-2">
          <div className="rounded-[1.5rem] bg-[rgba(31,74,64,0.08)] p-5">
            <p className="text-sm font-bold text-[#665950]">월 예상 비용</p>
            <p className="mt-2 text-3xl font-black">{result.monthlyTotal.toLocaleString("ko-KR")}원</p>
          </div>
          <div className="rounded-[1.5rem] bg-[rgba(255,184,107,0.12)] p-5">
            <p className="text-sm font-bold text-[#665950]">연 예상 비용</p>
            <p className="mt-2 text-3xl font-black">{result.yearlyTotal.toLocaleString("ko-KR")}원</p>
          </div>
        </div>
        <div className="mt-6 space-y-3">
          {result.breakdown.map((entry) => (
            <div key={entry.label} className="rounded-[1.4rem] border border-[rgba(56,41,29,0.08)] bg-white/80 p-4 text-sm">
        ...
        <p className="mt-5 text-sm leading-7 text-[#665950]">{result.tip}</p>
      </section>
