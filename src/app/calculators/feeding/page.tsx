    ...
    <main className="mx-auto max-w-4xl px-5 py-8 sm:py-10">
      <section className="section-shell px-6 py-6 sm:px-8 sm:py-8">
        <div className="relative z-10 max-w-3xl">
          ...
          <p className="mt-4 text-sm leading-8 text-[var(--muted)] sm:text-base">몸무게와 활동량, 사료 열량을 기준으로 하루 급여량과 1회 급여량을 계산합니다.</p>
        </div>
      </section>
      ...
      <form className="mt-6 grid gap-4 rounded-[1rem] card p-6 md:grid-cols-2">
        ...
        <label className="space-y-2 text-sm font-bold text-[var(--ink)]">나이(년)<input type="number" step="0.1" name="ageYears" defaultValue={input.ageYears} className="input" /></label>
        <label className="space-y-2 text-sm font-bold text-[var(--ink)]">중성화 여부<select name="neutered" defaultValue={input.neutered} className="input"><option value="yes">예</option><option value="no">아니오</option></select></label>
        <label className="space-y-2 text-sm font-bold text-[var(--ink)]">활동량<select name="activityLevel" defaultValue={input.activityLevel} className="input"><option value="low">낮음</option><option value="normal">보통</option><option value="high">높음</option></select></label>
        <label className="space-y-2 text-sm font-bold text-[var(--ink)]">사료 열량(kcal/100g)<input type="number" step="1" name="foodKcalPer100g" defaultValue={input.foodKcalPer100g} className="input" /></label>
      ...
      <section className="mt-6 card rounded-[1rem] p-6">
        <div className="grid gap-4 md:grid-cols-3">
          <div className="stat-tile"><p className="text-xs font-black text-[var(--muted)]">하루 필요 kcal</p><p className="mt-2 text-2xl font-black">{result.dailyKcal}</p></div>
          <div className="stat-tile"><p className="text-xs font-black text-[var(--muted)]">하루 급여량</p><p className="mt-2 text-2xl font-black">{result.dailyGrams}g</p></div>
          <div className="stat-tile"><p className="text-xs font-black text-[var(--muted)]">2회 급여 기준</p><p className="mt-2 text-2xl font-black">{result.perMealGrams}g</p></div>
        </div>
        <ul className="mt-6 space-y-2 text-sm leading-7 text-[var(--muted)]">
