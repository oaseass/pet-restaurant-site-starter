...
## 114 전화번호부 느낌 제거 및 장소 정체성 구조 개선 결과

1. 114 전화번호부처럼 보였던 원인
- 검색, 목록, 지도 카드가 `업체명 → 주소 → 전화/지도/상세 버튼` 순서로 반복되어 장소 소개보다 연락처 목록처럼 보였습니다.
- 상세 상단도 이름, 주소, 전화번호, 업데이트, 정보 기준을 그리드로 나열해 사용자가 눌렀을 때 `여기가 어떤 곳인지`보다 `어떤 데이터가 있는지`를 먼저 읽게 했습니다.
- 식당, 병원, 약국, 미용, 유치원·호텔, 장례가 서로 다른 방문 판단 기준을 갖는데도 카드 구조가 거의 같아 대표 메뉴, 진료 성격, 약품 재고, 예약 조건, 장례 절차 같은 맥락이 약했습니다.
- 사진이 없는 상태에서 아무 시각 신호가 없어 카드가 흰 박스와 텍스트 묶음으로만 보였습니다.

2. 제거/통합한 의미 없는 카드와 섹션
- 식당 상세의 `방문 전 확인 체크리스트`는 `강아지랑 가기 전 체크할 점`과 내용이 겹쳐 제거했습니다.
- 상세 판단 패널에서 `업데이트`, `정보 기준`을 각각 별도 fact 카드로 반복하던 구조를 줄이고, `확인된 것과 비어 있는 것` 안내로 통합했습니다.
- 카드 상단에서 출처/상태 배지를 먼저 나열하던 흐름을 낮추고, `장소 성격`, `한 줄 설명`, `대표 메뉴/서비스 빈칸`을 먼저 읽히게 재배치했습니다.
- 홈 카테고리 카드의 건수 중심 노출은 줄이고 `강아지랑 갈 곳`, `진료 가능한 곳`, `약 물어볼 곳` 같은 이용 목적을 먼저 보이게 했습니다.

3. 카드 구조를 어떻게 바꿨는지
- `src/lib/discovery-cards.ts`에 `getRestaurantIdentity`, `getPlaceIdentity`, `simplifyExternalCategory`를 추가해 카테고리별 장소 성격, 한 줄 설명, 부족한 정보 CTA, 시각 블록 종류를 공통 계산합니다.
- `src/components/discovery/CategoryVisualBlock.tsx`를 추가해 실제 업체 사진처럼 보이지 않는 카테고리별 시각 블록을 만들었습니다. 모든 블록에는 `실제 사진 아님`을 표시하고 `사진 제보하기` 흐름을 제공합니다.
- 식당 카드는 Kakao 외부 분류가 있으면 `한식`, `육류,고기`, `카페` 같은 장소 성격을 먼저 보여주고, 없으면 `반려동물 동반 식당`으로 보수적으로 설명합니다.
- 병원 카드는 업체명/외부 분류 기반으로 `동물병원`, `동물의료센터`, `24시 후보 병원`, `가축병원` 성격을 구분합니다. 야간·응급 여부는 단정하지 않습니다.
- 약국, 미용, 유치원·호텔, 장례 카드는 각각 재고 전화 확인, 견종·크기 예약 조건, 호텔·위탁·훈련 구분, 화장·봉안·픽업 상담 문구를 우선 표시합니다.
- 적용 범위: `RestaurantCard`, `PlaceCard`, `SearchResultsList`, `PlaceListSection`, `MapListPanel`, `MapBottomSheet`, `MapShell`, 지도 데이터 생성 흐름.

4. 상세 페이지에서 “어떤 곳인지”를 어떻게 보여주게 했는지
- 식당 상세 상단은 `장소 소개`, 업체명, 한 줄 설명, 장소 성격, 전화, 대표 메뉴, 동반 좌석, 후기, 지도/길찾기/후기/정보 CTA가 1스크린 안에 들어오도록 재구성했습니다.
- 장소 상세 상단은 병원/약국/미용/유치원·호텔/장례별로 `장소 소개`, 한 줄 설명, 전화, 주소, 진료 항목·취급 약품·서비스 빈칸, 후기, 지도/길찾기/제보 CTA를 먼저 보여줍니다.
- `GROOMING`/`DAYCARE` 상세에서는 Kakao enrichment를 실제 정보로 반영하지 않도록 `reliableEnrichment`를 null 처리했습니다.
- 병원/약국/장례 상세에는 확인된 Kakao 외부정보가 있는 경우에만 지도 분류와 외부 지도 링크를 표시합니다.

5. 사진/메뉴/서비스 정보가 없는 경우 어떻게 처리했는지
- 외부 사진, 메뉴판, 블로그, 리뷰를 무단 크롤링하거나 저장하지 않았습니다.
- 사진이 없는 경우 실제 업체 사진처럼 보이지 않는 카테고리별 시각 블록만 보여주고, `사진 제보하기` CTA를 제공합니다.
- 식당 상세에는 `대표 메뉴는 아직 등록되지 않았어요`와 `대표 메뉴 제보하기`, 외부 지도 링크가 있으면 `외부 지도에서 확인`을 제공합니다.
- 병원/약국/미용/유치원·호텔/장례는 `진료 항목`, `취급 약품`, `서비스`, `비용과 절차`를 단정하지 않고 제보 CTA와 전화 확인 흐름으로 처리했습니다.

6. npm test 결과
- `npm test`: 통과
- 결과: 27개 테스트 통과, 실패 0개
- 신규 테스트: 식당 외부 분류 우선순위, 병원/약국/유치원·호텔 장소 정체성 helper 분기

7. npm run build 결과
- `npm run build`: 통과
- Next.js 15.5.15 production build, Prisma Client generate, public data export, 타입 검사 모두 통과했습니다.
- build가 재생성한 `public/data/**` 변경은 커밋 대상에서 제외했습니다.

8. production 확인 결과
- 배포: `https://pet-restaurant-site-starter-ecizy08yl-larchides-projects.vercel.app`
- alias: `https://pet-restaurant-site-starter.vercel.app`
- 390px 모바일 폭으로 `/`, `/search?q=동물병원`, `/search?q=동물약국`, `/restaurants`, `/hospitals`, `/pharmacy`, `/grooming`, `/daycare`, `/funeral`, `/map?lat=35.190605&lng=126.815636&category=all`, `/restaurants/9bbc43ee-d9c5-4fac-a593-116d3cced8d6`, `/places/33e00bc4-44eb-4e88-a866-e1a3a868832b`, `/places/bd5eeb9b-cfb6-4bcb-a9fe-38f9a7556b31`, `/places/6f1e8b76-e33e-4dad-9692-05bc9bddb9a7`, `/lost-pets`, `/guide/travel` 확인 완료.
- 확인 경로 모두 200 응답, 가로 overflow 없음, 개발자용 enum 노출 없음, `내부 DB`, `관리자 검수형`, `상세보기`, `전화 제보`, `전화번호 제보`, `확인 필요`, `더미 사진`, `가짜 메뉴` 노출 없음.

9. 아직 남은 문제
- 실제 사진, 대표 메뉴, 진료 항목, 약품 재고, 장례 비용 같은 고밀도 정보는 아직 검증된 소스가 없어 제보 CTA와 전화 확인 문장으로 남겨두었습니다.
- 목록 상단의 지역별 건수 배지는 여전히 행정 데이터 느낌이 조금 남습니다. 다만 탐색에 필요한 신호라 이번에는 카드 본문 구조 개선을 우선했습니다.
- 카테고리별 시각 블록은 실제 사진을 대체하는 안내 장치일 뿐, 업체별 분위기를 보여주는 정보는 아닙니다.

10. 다음 개선 제안
- 제보/업체 등록 흐름에 `대표 메뉴`, `진료 항목`, `취급 약품`, `미용 가능 견종`, `호텔링 조건`, `장례 서비스 항목`, `사진`을 구조화 입력으로 받으면 카드와 상세가 더 장소답게 바뀝니다.
- 승인 후기에서 `좌석`, `대형견`, `예약`, `약품 재고`, `픽업 가능` 같은 신호를 뽑아 카드의 1줄 요약으로 연결하면 전화번호부 느낌을 더 줄일 수 있습니다.
- 지역별 건수 배지는 다음 라운드에서 `지금 전화해볼 곳`, `지도에서 바로 볼 곳`, `후기가 있는 곳` 같은 생활형 필터로 바꾸는 것이 좋습니다.
